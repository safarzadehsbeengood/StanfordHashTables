...
class CuckooHashTable {
  ...
  */
  void print() const {
    std::cout << std::setw(8) << "t1" << std::setw(10) << "" << std::setw(8) << "t2" << std::endl << std::setfill('-') << std::setw(32) << "" << std::setfill(' ') << std::endl;
    for (size_t i = 0; i < size / 2; i++) {
      std::cout << std::setw(8) << t1[i] << std::setw(10) << "" << std::setw(8) << t2[i] << std::endl;
    }
  }
...
private:
  ...
  std::vector<int> t1;
  std::vector<int> t2;
  static const size_t MAX_RETRIES = 100;
  size_t size;

  bool cuckooInsert(int data, int key1, int key2, size_t retries);
